        ...
        <v-row class="justify-center">
            ...
            <v-col cols="5">
                <v-row class="justify-right">
                    <v-card class="bg-card-on-surface">
                        <v-card-title>Unique Members</v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    {{ uniqueMemberCount }} in last 90 days
                                </v-col>
                            </v-row>
                        </v-card-text>
                    </v-card>
...
const uniqueMemberCount = ref(0)

async function fetchUniqueMemberCount() {
    try {
        uniqueMemberCount.value = await attendanceStore.getUniqueMembers()
    } catch (error) {
        console.error(error)
    }
}
